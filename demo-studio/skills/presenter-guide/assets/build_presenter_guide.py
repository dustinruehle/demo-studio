...
def build(cfg):
    ...
    nav_items=[]
    for s in slides:
        label, color = ACT.get(s["act"], (s["act"], "#4C2889"))
        nav_items.append(
...
:root{
  ...
  --amber:#B07A00;--green:#2E7D5B;--green-tint:#E8F2ED;--coral:#C0503A;--coral-tint:#FBEDE8;
  --muted:#6A6478;--faint:#8B84A3;--line:#E4E0EC;
}
...
/* questions */
.qcard{background:var(--coral-tint);border:1px solid #F0D8CE;border-radius:8px;padding:10px 12px;font-size:13.5px;color:var(--ink);line-height:1.4;margin-bottom:8px;}
...
.lane.work{border-left:3px solid var(--coral);} .lane.work .ln{color:#B24326;}
...
.smoke{background:var(--green-tint);border:1px solid #CDE7DB;border-radius:8px;padding:11px 13px;font-size:13.5px;color:#245A44;margin:12px 0;line-height:1.5;}
...
.term-action{font-family:"JetBrains Mono",monospace;font-size:12.5px;background:#241C33;color:#F4F1FB;border-radius:8px;padding:10px 13px;margin:8px 0 12px;line-height:1.5;}
.term-action code{background:rgba(255,255,255,.14);padding:1px 5px;border-radius:4px;color:#fff;}
.term-action b{color:#F5A38A;}
...
.switchblock{background:#241C33;border-radius:9px;padding:13px 15px;}
.switchblock .sw-t{font-family:"JetBrains Mono",monospace;font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#C9B8F0;margin-bottom:8px;}
.switchblock pre{margin:0;font-family:"JetBrains Mono",monospace;font-size:11.5px;color:#E7E2F5;white-space:pre-wrap;line-height:1.55;}
